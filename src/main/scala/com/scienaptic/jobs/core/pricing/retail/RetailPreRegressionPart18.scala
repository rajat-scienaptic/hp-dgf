...
object RetailPreRegressionPart18 {
  ...
  def execute(executionContext: ExecutionContext): Unit = {
    ...
    var retailGroupWEDL1InnerCompCann3  = executionContext.spark.read.option("header", true).option("inferSchema", true).csv("E:\\Scienaptic\\HP\\Pricing\\Data\\CR1\\May31_Run\\spark_out_retail\\retail-L1L2InnerCann-PART17.csv")
    ...
    retailGroupWEDL1InnerCompCann3.coalesce(1).write.option("header", true).mode(SaveMode.Overwrite).csv("E:\\Scienaptic\\HP\\Pricing\\Data\\CR1\\May31_Run\\spark_out_retail\\retail-PriceBandCannOfflineOnline-PART18.csv")
