...
object CommercialFeatEnggProcessor {
  ...
  def execute(executionContext: ExecutionContext): Unit = {
    ...
    //commercial.coalesce(1).write.option("header","true").mode(SaveMode.Overwrite).csv("C:\\Users\\avika\\Downloads\\JarCode\\R Code Inputs\\outputs\\commercial.csv")
    ...
    ifs2 = ifs2
      //.withColumn("Valid_Start_Date", when(col("Valid_Start_Date").isNull || col("Valid_Start_Date") === "", null).otherwise(to_date(unix_timestamp(convertFaultyDateFormat(col("Valid_Start_Date")), "yyyy-MM-dd").cast("timestamp"))))
    ...
    //ifs2.coalesce(1).write.option("header","true").mode(SaveMode.Overwrite).csv("C:\\Users\\avika\\Downloads\\JarCode\\R Code Inputs\\outputs\\ifs2DF.csv")
    /*val ifs2 = spark.read.option("header","true").option("inferSchema","true").csv("C:\\Users\\avika\\Downloads\\JarCode\\R Code Inputs\\outputs\\ifs2.csv")
    val commercial = spark.read.option("header","true").option("inferSchema","true").csv("C:\\Users\\avika\\Downloads\\JarCode\\R Code Inputs\\outputs\\commercial.csv")
    ...
    //val commercialMergeifs2DF = spark.read.option("header","true").option("inferSchema","true").csv("C:\\Users\\avika\\Downloads\\JarCode\\R Code Inputs\\outputs\\commercialMergeifs2DF.csv")
    */val ifs2 = spark.read.option("header","true").option("inferSchema","true").csv("C:\\Users\\avika\\Downloads\\JarCode\\R Code Inputs\\outputs\\ifs2.csv")
...
//Insight Direct
    ...
    stockpiler_dealchaser = stockpiler_dealchaser
      ...
      //TODO: Check if this result matches with Sandeepan! IR might be a problem here
    writeDF(commercialSpclPrgmFactDF,"commercialSpclPrgmFactDF")
    writeDF(stockpiler_dealchaser,"stockpiler_dealchaser")

    /*val stockpiler_dealchaser = spark.read.option("header","true").option("inferSchema","true").csv("C:\\Users\\avika\\Downloads\\JarCode\\R Code Inputs\\outputs\\stockpiler_dealchaser.csv")
    val commercialSpclPrgmFactDF = spark.read.option("header","true").option("inferSchema","true").csv("C:\\Users\\avika\\Downloads\\JarCode\\R Code Inputs\\outputs\\commercialSpclPrgmFactDF.csv")
      .withColumn("GA date",to_date(col("GA date")))
      .withColumn("ES date",to_date(col("ES date")))
      .withColumn("Week_End_Date",to_date(col("Week_End_Date")))*/
    ...
    //println("unique values ::::::::::::::::::::::::: " +stockpilerResellerList)
    val commercialMatResellerStockPilerDF = commercialSpclPrgmFactDF//.join(stockpiler_dealchaser.select("Reseller_Cluster","proportion_on_promo"), Seq("Reseller_Cluster"), "left")
    ...
    val commercialResellerClusterFactDF = pipelineForResellerCluster.fit(commercialMatResellerStockPilerDF).transform(commercialMatResellerStockPilerDF)
      ...
      .drop("eTailer")//.withColumn("eTailer", lit(null).cast("string"))
    ...
    //TODO: agg(sum(QtY)) to agg(sum(Qty)) -> Y should not be capital -- Checked
    var commercialHPDF = commercialResFacNotPL
      ...
      //.withColumnRenamed("CategorySubgroup","Category Subgroup")
      //.withColumnRenamed("CategoryCustom","Category Custom")
      //.withColumnRenamed("GAdate","GA date")
      //.withColumnRenamed("ESdate","ES date")
    ...
    //TODO: Number of rows not matching. So, 'IR' not matching. Creating problem in 'Promo_Flag' Line 215. -- Checked
    ...
    /*var commercialSSDF = spark.read.option("header","true").option("inferSchema","true").csv("C:\\Users\\avika\\Downloads\\JarCode\\R Code Inputs\\outputs\\commercialSSAfterGroup.csv")
      .withColumn("Week_End_Date",to_date(col("Week_End_Date"))).cache()
    var commercialHPDF = spark.read.option("header","true").option("inferSchema","true").csv("C:\\Users\\avika\\Downloads\\JarCode\\R Code Inputs\\outputs\\commercialHPAfterGroup.csv")
      .withColumn("GA date",to_date(col("GA date")))
      .withColumn("ES date",to_date(col("ES date")))
      .withColumn("Week_End_Date",to_date(col("Week_End_Date"))).cache()
    ...
    commercialSSFactorColumnsList.foreach(column => {
      ...
      val pipeline = new Pipeline().setStages(Array(indexer))
      commercialSSFactorsDF = commercialSSFactorsDF.withColumn(column+"_LEVELS", col(column))  //TODO: Change this to col(column)
    ...
    //TODO- break here!
    /*var commercialSSFactorsDF = spark.read.option("header","true").option("inferSchema","true").csv("C:\\Users\\avika\\Downloads\\JarCode\\R Code Inputs\\outputs\\commercialSSFactorsDF_After_StringInd_Many_Vars.csv")
    var commercialHPDF = spark.read.option("header","true").option("inferSchema","true").csv("C:\\Users\\avika\\Downloads\\JarCode\\R Code Inputs\\outputs\\commercialHPDF_After_StringInd_Many_Vars.csv")
      .withColumn("GA date_LEVELS",to_date(col("GA date_LEVELS")))
      .withColumn("ES date_LEVELS",to_date(col("ES date_LEVELS")))
      .withColumn("Week_End_Date",to_date(col("Week_End_Date"))).cache()*/
    ...
    var commercialHPandSSDF = spark.read.option("header","true").option("inferSchema","true").csv("C:\\Users\\avika\\Downloads\\JarCode\\R Code Inputs\\outputs\\commercialHPandSSDF.csv")
      .withColumn("GA date_LEVELS",to_date(col("GA date_LEVELS")))
      .withColumn("ES date_LEVELS",to_date(col("ES date_LEVELS")))
      .withColumn("Week_End_Date",to_date(col("Week_End_Date"))).cache()
    ...
    /*var commercialwithHolidaysDF = spark.read.option("header","true").option("inferSchema","true").csv("C:\\Users\\avika\\Downloads\\JarCode\\R Code Inputs\\outputs\\Commercial_With_Holidays.csv")
      .withColumn("GA date_LEVELS",to_date(col("GA date_LEVELS")))
      .withColumn("ES date_LEVELS",to_date(col("ES date_LEVELS")))
      .withColumn("Week_End_Date",to_date(col("Week_End_Date")))
    */
    ...
    writeDF(npd,"npd")*/
    var npd = spark.read.option("header","true").option("inferSchema","true").csv("C:\\Users\\avika\\Downloads\\JarCode\\R Code Inputs\\outputs\\npd.csv")
      .withColumn("Week_End_Date",to_date(col("Week_End_Date")))
    /*
    ...
    /*====================================== Brand Not HP =================================*/
    /*var commercialWithCompetitionDF = spark.read.option("header","true").option("inferSchema","true").csv("C:\\Users\\avika\\Downloads\\JarCode\\R Code Inputs\\outputs\\commercialWithCompetitionDF_WITH_L1_L2.csv")
      .withColumn("GA date_LEVELS",to_date(col("GA date_LEVELS")))
      .withColumn("ES date_LEVELS",to_date(col("ES date_LEVELS")))
      .withColumn("Week_End_Date",to_date(col("Week_End_Date")))
    ...
    //L2CompetitionSS.coalesce(1).write.option("header","true").mode(SaveMode.Overwrite).csv("C:\\Users\\avika\\OneDrive\\Desktop\\HP Docs\\Pricing\\Pipeline_Test_Data\\outputs\\pre-regression_data_commercial.csv")
    //L2CompetitionSS.coalesce(1).write.option("header","true").mode(SaveMode.Overwrite).csv("C:\\Users\\avika\\OneDrive\\Desktop\\HP Docs\\Pricing\\Pipeline_Test_Data\\outputs\\pre-regression_data_commercial_"+java.time.LocalDate.now.toString+".csv")
    ...
    /*var commercialWithCompetitionDF = spark.read.option("header","true").option("inferSchema","true").csv("C:\\Users\\avika\\Downloads\\JarCode\\R Code Inputs\\outputs\\commercialWithCompetitionDF_Samsung.csv")
      .withColumn("GA date_LEVELS",to_date(col("GA date_LEVELS")))
      .withColumn("ES date_LEVELS",to_date(col("ES date_LEVELS")))
      .withColumn("Week_End_Date",to_date(col("Week_End_Date")))
    val commercialBrandinHP = commercialWithCompetitionDF.where(col("Brand").isin("HP"))  //TODO: Change for PMAX implementation
          //.withColumn("Qty_pmax",when(col("Qty")<0,0).otherwise(col("Qty")))
    ...
    * */
    /*var commercialWithCompetitionDF = spark.read.option("header","true").option("inferSchema","true").csv("C:\\Users\\avika\\Downloads\\JarCode\\R Code Inputs\\outputs\\commercialWithCompetitionDF_L1_L2_Competition_SS_Feat.csv")
      .withColumn("GA date_LEVELS",to_date(col("GA date_LEVELS")))
      .withColumn("ES date_LEVELS",to_date(col("ES date_LEVELS")))
      .withColumn("Week_End_Date",to_date(col("Week_End_Date")))
    commercialWithCompetitionDF = commercialWithCompetitionDF//.withColumn("wed_cat", concat_ws(".",col("Week_End_Date"), col("L1_Category")))*/
      //.withColumn("Qty", when(col("Qty")<0, 0).otherwise(col("Qty")))   //TODO: DO NOT UNCOMMENT!!
    ...
    //var commercialWithCompCannDF = spark.read.option("header","true").option("inferSchema","true").csv("C:\\Users\\avika\\Downloads\\JarCode\\R Code Inputs\\outputs\\commercialWithCompetitionDF_HP_Competitions.csv")
    //DON'T remove join
    ...
    //commercialWithCompCannDF.write.option("header","true").mode(SaveMode.Overwrite).csv("C:\\Users\\avika\\Downloads\\outputs\\commercialWithCompCannDF.csv")
    //commercialGroupWEDSKU.write.option("header","true").mode(SaveMode.Overwrite).csv("C:\\Users\\avika\\Downloads\\outputs\\commercialGroupWEDSKU.csv")
    //commercialGroupWEDL1Temp.write.option("header","true").mode(SaveMode.Overwrite).csv("C:\\Users\\avika\\Downloads\\outputs\\ommercialGroupWEDL1Temp.csv")

    //var commercialGroupWEDL1Temp = spark.read.option("header","true").option("inferSchema","true").csv("C:\\Users\\avika\\Downloads\\outputs\\ommercialGroupWEDL1Temp.csv")
    //var commercialGroupWEDSKU = spark.read.option("header","true").option("inferSchema","true").csv("C:\\Users\\avika\\Downloads\\outputs\\commercialGroupWEDSKU.csv")
    //var commercialWithCompCannDF = spark.read.option("header","true").option("inferSchema","true").csv("C:\\Users\\avika\\Downloads\\outputs\\commercialWithCompCannDF.csv")
      //TODO: Done till here!
      //join_both = df1.join(df2, (col("df1.Week_End_Date") === col("df2.Week_End_Date")) && (col("df1.Brand") === col("df2.Brand")) && (col("df1.L1_Category") === col("df2.L1_Category")), "left")

    val commercialGroupWEDL1 = commercialGroupWEDSKU.withColumn("L1_Category",col("L1_Category")).join(commercialGroupWEDL1Temp.withColumn("L1_Category",col("L1_Category")), Seq("Week_End_Date","Brand", "L1_Category"), "left")
    ...
    /*var commercialWithCompCannDF = spark.read.option("header","true").option("inferSchema","true").csv("C:\\Users\\avika\\Downloads\\JarCode\\R Code Inputs\\outputs\\commercialWithCompCannDF_CANNABILIZATION.csv")
      .withColumn("GA date_LEVELS",to_date(col("GA date_LEVELS")))
      .withColumn("ES date_LEVELS",to_date(col("ES date_LEVELS")))
      .withColumn("Week_End_Date",to_date(col("Week_End_Date"))).cache()*/
    ...
    //writeDF(npdChannelNotRetail,"npdChannelNotRetail_WITH_WEEK_EXTRACTED")
    ...
    //TODO: Mismatch occurs here!!
    //writeDF(seasonalityNPD,"seasonalityNPD_First")
    ...
    //val indexerForL1 = new StringIndexer().setInputCol("L1_Category").setOutputCol("L1_Category_fact").setHandleInvalid("keep")
    //val pipelineForL1 = new Pipeline().setStages(Array(indexerForL1))
    seasonalityNPDScanner = seasonalityNPDScanner/*pipelineForL1.fit(seasonalityNPDScanner).transform(seasonalityNPDScanner)*/
    ...
    //var seasonalityNPD = spark.read.option("header","true").option("inferSchema","true").csv("C:\\Users\\avika\\Downloads\\JarCode\\R Code Inputs\\outputs\\seasonalityNPD.csv")
    commercialWithCompCannDF = commercialWithCompCannDF.join(seasonalityNPD, Seq("L1_Category","Week"), "left")
    //TODO: Write Join of commercial and SeasonalityNPD
        .drop("Week")
    ...
    /*var commercialWithCompCannDF = spark.read.option("header","true").option("inferSchema","true").csv("C:\\Users\\avika\\Downloads\\JarCode\\R Code Inputs\\outputs\\commercialWithCompCannDF_Seasonality.csv")
      .withColumn("GA date_LEVELS",to_date(col("GA date_LEVELS")))
      .withColumn("ES date_LEVELS",to_date(col("ES date_LEVELS")))
      .withColumn("Week_End_Date",to_date(col("Week_End_Date"))).cache()*/
    ...
    /*var commercialWithCompCannDF = spark.read.option("header","true").option("inferSchema","true").csv("C:\\Users\\avika\\Downloads\\JarCode\\R Code Inputs\\outputs\\commercialWithCompCannDF_WITH_HARDWARE_SUPPLIES_FEAT.csv")
      .withColumn("GA date_LEVELS",to_date(col("GA date_LEVELS")))
      .withColumn("ES date_LEVELS",to_date(col("ES date_LEVELS")))
      .withColumn("Week_End_Date",to_date(col("Week_End_Date")))
    ...
    /*var commercialWithCompCannDF = spark.read.option("header","true").option("inferSchema","true").csv("C:\\Users\\avika\\Downloads\\JarCode\\R Code Inputs\\outputs\\commercialWithCompCannDF_WITH_LOGS.csv")
      .withColumn("Week_End_Date",to_date(col("Week_End_Date")))
      .withColumn("ES date_LEVELS",to_date(col("ES date_LEVELS")))
      .withColumn("GA date_LEVELS",to_date(col("GA date_LEVELS")))
    ...
    //val percentil75DF = spark.sql("select SKU_Name, Reseller_Cluster, Reseller_Cluster_LEVELS, percentile_approx(Qty,0.75) as percentile_0_75 from commercial group by SKU_Name, Reseller_Cluster, Reseller_Cluster_LEVELS")
    ...
    commercialWithCompCannDF.createOrReplaceTempView("commercial")
    //val percentile25DF = spark.sql("select SKU_Name, Reseller_Cluster, Reseller_Cluster_LEVELS, percentile_approx(Qty,0.25) as percentile_0_25 from commercial group by SKU_Name, Reseller_Cluster, Reseller_Cluster_LEVELS")
    ...
    /*var commercialWithCompCannDF = spark.read.option("header","true").option("inferSchema","true").csv("C:\\Users\\avika\\Downloads\\JarCode\\R Code Inputs\\outputs\\commercialWithCompCannDF_Spike.csv")
      .withColumn("Week_End_Date",to_date(col("Week_End_Date")))
      .withColumn("ES date_LEVELS",to_date(col("ES date_LEVELS")))
      .withColumn("GA date_LEVELS",to_date(col("GA date_LEVELS"))).cache()
    ...
    //writeDF(commercialWithHolidayAndQtyFilter,"commercialWithHolidayAndQtyFilter_FILTER_FOR_NPBL")
      //TODO: Convert Qty as numeric: Remove precision value.
    ...
    //var npbl = spark.read.option("header","true").option("inferSchema","true").csv("C:\\Users\\avika\\Downloads\\JarCode\\R Code Inputs\\outputs\\npbl_BEFORE_MEDIAN.csv")
    ...
    //val npblTemp = spark.sql("select Reseller_Cluster, Reseller_Cluster_LEVELS, SKU_Name, percentile_approx(Qty, 0.5) as no_promo_med from npbl group by Reseller_Cluster,Reseller_Cluster_LEVELS,SKU_Name")
    ...
    /*TODO: NOT REQUIRED!!!! npbl = commercialWithHolidayAndQtyFilter.withColumn("SKU_Name",col("SKU_Name")).withColumn("Reseller_Cluster",col("Reseller_Cluster"))
      .join(npbl.withColumn("SKU_Name",col("SKU_Name")).withColumn("Reseller_Cluster",col("Reseller_Cluster")), Seq("Reseller_Cluster","SKU_Name","Reseller_Cluster_LEVELS"), "left")*/
    //writeDF(npbl,"npbl")
    //var npbl = spark.read.option("header","true").option("inferSchema","true").csv("C:\\Users\\avika\\Downloads\\JarCode\\R Code Inputs\\outputs\\npbl.csv")
    commercialWithCompCannDF = commercialWithCompCannDF.withColumn("SKU_Name",col("SKU_Name")).withColumn("Reseller_Cluster",col("Reseller_Cluster"))
    ...
    /*var commercialWithCompCannDF = spark.read.option("header","true").option("inferSchema","true").csv("C:\\Users\\avika\\Downloads\\JarCode\\R Code Inputs\\outputs\\commercialWithCompCannDF_AFTER_NPBL.csv")
      .withColumn("Week_End_Date",to_date(col("Week_End_Date")))
      .withColumn("ES date_LEVELS",to_date(col("ES date_LEVELS")))
      .withColumn("GA date_LEVELS",to_date(col("GA date_LEVELS")))*/
...
//var EOLcriterion = spark.read.option("header","true").option("inferSchema","true").csv("C:\\Users\\avika\\Downloads\\JarCode\\R Code Inputs\\outputs\\EOLcriterion_FIRST.csv")
  //.withColumn("Week_End_Date",to_date(col("Week_End_Date"))).cache()
// var commercialWithCompCannDF = spark.read.option("header","true").option("inferSchema","true").csv("C:\\Users\\avika\\Downloads\\outputs\\commercialWithCompCannDF.csv")
...
/*var EOLcriterion = spark.read.option("header","true").option("inferSchema","true").csv("C:\\Users\\avika\\Downloads\\JarCode\\R Code Inputs\\outputs\\EOLcriterion_WITH_RANK.csv")
  .withColumn("Week_End_Date",to_date(col("Week_End_Date"))).cache()
...
//EOLWithCriterion1.where(col("SKU&Reseller").contains("Sprocket") && col("SKU&Reseller").contains("Option B")).show(20,false)
...
//var EOLcriterion = spark.read.option("header","true").option("inferSchema","true").csv("C:\\Users\\avika\\Downloads\\JarCode\\R Code Inputs\\outputs\\EOLcriterion_BEFORE_LAST_MIN_MAX.csv")
val EOLCriterionLast = EOLcriterion.where(col("EOL_criterion")===lit(1))
.groupBy("SKU_Name","Reseller_Cluster","Reseller_Cluster_LEVELS")
.agg(max("Week_End_Date").as("last_date"))
//.join(EOLcriterion.where(col("EOL_criterion")===1), Seq("SKU_Name","Reseller_Cluster","Reseller_Cluster_LEVELS"), "right")
...
/*val EOLCriterionLast = spark.read.option("header","true").option("inferSchema","true").csv("C:\\Users\\avika\\Downloads\\JarCode\\R Code Inputs\\outputs\\EOLCriterionLast.csv")
  .withColumn("last_date",to_date(col("last_date")))
var EOLCriterionMax = spark.read.option("header","true").option("inferSchema","true").csv("C:\\Users\\avika\\Downloads\\JarCode\\R Code Inputs\\outputs\\EOLCriterionMax.csv")
  .withColumn("max_date",to_date(col("max_date")))*/
    /*Remove 'var'*/
...
//var EOLcriterion = spark.read.option("header","true").option("inferSchema","true").csv("C:\\Users\\avika\\Downloads\\JarCode\\R Code Inputs\\outputs\\EOLcriterion_BEFORE_MAXMAXDAte.csv").withColumn("max_date",to_date(col("max_date")))/*Remove this*/.withColumn("last_date",lit(null).cast("date"))
...
//var EOLcriterion = spark.read.option("header","true").option("inferSchema","true").csv("C:\\Users\\avika\\Downloads\\JarCode\\R Code Inputs\\outputs\\EOLcriterion_AFTER_MAXMAX.csv").cache()
...
/*var commercialWithCompCannDF = spark.read.option("header","true").option("inferSchema","true").csv("C:\\Users\\avika\\Downloads\\JarCode\\R Code Inputs\\outputs\\commercialWithCompCannDF_WITH_EOL.csv")
  .withColumn("ES date_LEVELS",to_date(col("ES date_LEVELS")))
  .withColumn("GA date_LEVELS",to_date(col("GA date_LEVELS")))
  .withColumn("Week_End_Date",to_date(col("Week_End_Date"))).cache()*/
...
/*  var BOL = spark.read.option("header","true").option("inferSchema","true").csv("C:\\Users\\avika\\Downloads\\JarCode\\R Code Inputs\\outputs\\BOL_WITH_WDAYs.csv")
    .withColumn("ES date",to_date(col("ES date")))
    .withColumn("GA date",to_date(col("GA date"))).cache()*/
BOL = BOL
  .withColumnRenamed("GA date","GA_date")
  .withColumnRenamed("ES date","ES_date")
  .withColumnRenamed("GA date_wday_sub","GA_date_wday_sub")
  .withColumnRenamed("ES date_wday_sub","ES_date_wday_sub")
  .withColumn("GA_date", expr("date_add(GA_date,GA_date_wday_sub)"))
  .withColumn("ES_date", expr("date_add(ES_date,ES_date_wday_sub)"))
  .withColumnRenamed("GA_date","GA date")
  .withColumnRenamed("ES_date","ES date")
  .withColumnRenamed("GA_date_wday_sub","GA date_wday_sub")
  .withColumnRenamed("ES_date_wday_sub","ES date_wday_sub")
...
/*var commercialWithCompCannDF = spark.read.option("header","true").option("inferSchema","true").csv("C:\\Users\\avika\\Downloads\\JarCode\\R Code Inputs\\outputs\\commercialWithCompCannDF_WITH_EOL.csv")
  ...
  .withColumn("GA date_LEVELS", to_date(col("GA date_LEVELS")))
  .withColumn("Week_End_Date", to_date(col("Week_End_Date")))
var BOL = spark.read.option("header","true").option("inferSchema","true").csv("C:\\Users\\avika\\Downloads\\JarCode\\R Code Inputs\\outputs\\BOL.csv")
  .withColumn("ES date", to_date(col("ES date")))
  .withColumn("GA date", to_date(col("GA date")))*/
...
/*var BOLCriterionFirst = spark.read.option("header","true").option("inferSchema","true").csv("C:\\Users\\avika\\Downloads\\JarCode\\R Code Inputs\\outputs\\BOLCriterionFirst.csv")
  .withColumn("first_date", to_date(col("first_date")))
var BOLCriterionMax = spark.read.option("header","true").option("inferSchema","true").csv("C:\\Users\\avika\\Downloads\\JarCode\\R Code Inputs\\outputs\\BOLCriterionMax.csv")
  .withColumn("max_date", to_date(col("max_date")))
var BOLCriterionMin = spark.read.option("header","true").option("inferSchema","true").csv("C:\\Users\\avika\\Downloads\\JarCode\\R Code Inputs\\outputs\\BOLCriterionMin.csv")
  .withColumn("min_date", to_date(col("min_date")))*/
...
/*var BOLCriterion = spark.read.option("header","true").option("inferSchema","true").csv("C:\\Users\\avika\\Downloads\\JarCode\\R Code Inputs\\outputs\\BOLCriterion_AFTER_FIRST_MIN_MAX_JOIN.csv")
  .withColumn("max_date",to_date(col("max_date")))
  .withColumn("first_date",to_date(col("first_date")))
  .withColumn("min_date",to_date(col("min_date"))).cache()
...
/*var BOLCriterion = spark.read.option("header","true").option("inferSchema","true").csv("C:\\Users\\avika\\Downloads\\JarCode\\R Code Inputs\\outputs\\BOLCriterion_WITH_DIFF_WEEKS.csv")
  //.withColumn("max_date",to_date(col("max_date")))
  .withColumn("first_date",to_date(col("first_date")))
  .withColumn("min_date",to_date(col("min_date"))).cache()
BOLCriterion = BOLCriterion
...
//var commercialWithCompCannDF = spark.read.option("header","true").option("inferSchema","true").csv("C:\\Users\\avika\\Downloads\\JarCode\\R Code Inputs\\outputs\\commercialWithCompCannDF_BEFORE_BOL_JOIN.CSV")
  //.withColumn("ES date_LEVELS", to_date(col("ES date_LEVELS")))
  //.withColumn("GA date_LEVELS", to_date(col("GA date_LEVELS")))
  //.withColumn("Week_End_Date", to_date(col("Week_End_Date")))
/*var BOLCriterion = spark.read.option("header","true").option("inferSchema","true").csv("C:\\Users\\avika\\Downloads\\JarCode\\R Code Inputs\\outputs\\BOLCriterion_FINAL_DF.csv")
  .withColumn("first_date", to_date(col("first_date")))
  .withColumn("Week_End_Date", to_date(col("Week_End_Date")))*/
...
/*var commercialWithCompCannDF = spark.read.option("header","true").option("inferSchema","true").csv("C:\\Users\\avika\\Downloads\\JarCode\\R Code Inputs\\outputs\\commercialWithCompCannDF_WITH_BOL_JOINED_BEFORE_NULL_IMPUTATION.csv")
  .withColumn("ES date_LEVELS",to_date(col("ES date_LEVELS")))
  .withColumn("GA date_LEVELS",to_date(col("GA date_LEVELS")))
  .withColumn("Week_End_Date",to_date(col("Week_End_Date"))).cache()
commercialWithCompCannDF = commercialWithCompCannDF
...
/*var commercialPromoMean = spark.read.option("header","true").option("inferSchema","true").csv("C:\\Users\\avika\\Downloads\\JarCode\\R Code Inputs\\outputs\\commercialPromoMean_WITH_PROMO_FLAG_Avg.csv").cache()
var commercialWithCompCannDF = spark.read.option("header","true").option("inferSchema","true").csv("C:\\Users\\avika\\Downloads\\JarCode\\R Code Inputs\\outputs\\commercialWithCompCannDF_WITH_OPPOSITE.csv")
  .withColumn("ES date_LEVELS",to_date(col("ES date_LEVELS")))
  .withColumn("GA date_LEVELS",to_date(col("GA date_LEVELS")))
  .withColumn("Week_End_Date",to_date(col("Week_End_Date")))
  .withColumn("Valid_Start_Date",to_date(col("Valid_Start_Date")))
...
writeDF(commercialWithCompCannDF,"commercialWithCompCannDF_DIRECT_CANN_20")*/
    ...
    var commercialWithCompCannDF = spark.read.option("header","true").option("inferSchema","true").csv("C:\\Users\\avika\\Downloads\\JarCode\\R Code Inputs\\outputs\\commercialWithCompCannDF_DIRECT_CANN_20.csv")
      .withColumn("ES date_LEVELS",to_date(col("ES date_LEVELS")))
      .withColumn("GA date_LEVELS",to_date(col("GA date_LEVELS")))
      .withColumn("Week_End_Date",to_date(col("Week_End_Date")))
      .withColumn("Valid_Start_Date",to_date(col("Valid_Start_Date")))
...
writeDF(commercialWithCompCannDF,"commercialWithCompCannDF_EXCLUDE_BEFORE_SECOND_WHEN")
commercialWithCompCannDF = commercialWithCompCannDF
...
writeDF(commercialWithCompCannDF,"commercialWithCompCannDF_EXCLUDE_BEFORE_SPROKET")
commercialWithCompCannDF = commercialWithCompCannDF
  .withColumn("exclude", when(col("SKU_Name").contains("Sprocket"), 1).otherwise(col("exclude")))
writeDF(commercialWithCompCannDF,"commercialWithCompCannDF_EXCLUDE_FINAL")
...
writeDF(commercialWithCompCannDF,"commercialWithCompCannDF_EXCLUDE_SKUWHOCHANGE")
    ...
    /*var commercialWithCompCannDF = spark.read.option("header","true").option("inferSchema","true").csv("C:\\Users\\avika\\Downloads\\JarCode\\R Code Inputs\\outputs\\commercialWithCompCannDF_EXCLUDE_SKUWHOCHANGE.csv")
      .withColumn("ES date_LEVELS",to_date(col("ES date_LEVELS")))
      .withColumn("GA date_LEVELS",to_date(col("GA date_LEVELS")))
      .withColumn("Week_End_Date",to_date(col("Week_End_Date")))
      .withColumn("Valid_Start_Date",to_date(col("Valid_Start_Date")))
...
//commercialWithCompCannDF = commercialWithCompCannDF.withColumn("GA date",col("GA date").cast("string")).withColumn("ES date",col("ES date").cast("string"))
...
.drop(x).withColumnRenamed(x+"_fact",x).drop(x+"_fact")
    ...
    writeDF(commercialWithCompCannDF,"commercialWithCompCannDF_PRE-regression-commercial-output")
/*
