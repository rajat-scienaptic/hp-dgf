...
object USTransformer {
  ...
  def execute(executionContext: ExecutionContext): Unit = {
    ...
    val USMthReseller_stg  = spark.sql("select * from "+SANDBOX_DATAMART+"."+DM_PC_US_Mth_Reseller_STG)
      .withColumn("ams_source",lit("Reseller"))
    val USMthResellerBTO_stg  = spark.sql("select * from "+SANDBOX_DATAMART+"."+DM_PC_US_Mth_Reseller_BTO_STG)
      .withColumn("ams_source",lit("ResellerBTO"))
    val USMthDist_stg  = spark.sql("select * from "+SANDBOX_DATAMART+"."+DM_US_PC_Monthly_Dist_STG)
      .withColumn("ams_source",lit("Dist"))
    val USMthDistBTO_stg  = spark.sql("select * from "+SANDBOX_DATAMART+"."+DM_US_PC_Monthly_Dist_BTO_STG)
      .withColumn("ams_source",lit("DistBTO"))
    val USMthRetail_stg  = spark.sql("select * from "+SANDBOX_DATAMART+"."+DM_US_PC_Monthly_Retail_STG)
    ...
    USMthReseller_int.write.mode(SaveMode.Overwrite)
      .saveAsTable(SANDBOX_DATAMART+"."+"Int_Fact_DM_PC_US_Mth_Reseller");
    ...
    USMthResellerBTO_int.write.mode(SaveMode.Overwrite)
      .saveAsTable(SANDBOX_DATAMART+"."+"Int_Fact_DM_PC_US_Mth_Reseller_BTO");

    val USMthDist_int  = USMthDist_stg.transform(withAllTransformations)
      .withColumn("ams_source",lit("Dist"))

    USMthDist_int.write.mode(SaveMode.Overwrite)
      .saveAsTable(SANDBOX_DATAMART+"."+"Int_Fact_DM_US_PC_Monthly_Dist");
    ...
    USMthDistBTO_int.write.mode(SaveMode.Overwrite)
      .saveAsTable(SANDBOX_DATAMART+"."+"Int_Fact_DM_US_PC_Monthly_Dist_BTO");
