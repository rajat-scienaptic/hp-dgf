  ...
  def execute(executionContext: ExecutionContext): Unit = {
    ...
    retailWithCompCannDF = retailWithCompCannDF
      ...
      .withColumn("supplies_GM_scaling_factor", when(col("Account") === "Best Buy" && col("SKU") === "M9L66A", lit(-0.8))
        .when(col("Account") === "Best Buy" && col("SKU") === "T0F29A", lit(-0.6))
        .when(col("Account") === "Best Buy" && col("SKU") === "T0F28A", lit(-0.8)).otherwise(col("supplies_GM_scaling_factor")))
      .withColumn("supplies_GM_scaling_factor", when(col("Account") === "Office Depot-Max" && col("SKU") === "M9L66A", lit(-0.8))
        .when(col("Account") === "Office Depot-Max" && col("SKU") === "T0F29A", lit(-0.7))
        .when(col("Account") === "Office Depot-Max" && col("SKU") === "F1J03A", lit(-0.5)).otherwise(col("supplies_GM_scaling_factor")))
      .withColumn("supplies_GM_scaling_factor", when(col("Account") === "Staples" && col("SKU") === "T0F29A", lit(-0.7))
        .when(col("Account") === "Staples" && col("SKU") === "F8B04A", lit(-0.5))
        .when(col("Account") === "Staples" && col("SKU") === "E3E03A", lit(-0.5)).otherwise(col("supplies_GM_scaling_factor")))
