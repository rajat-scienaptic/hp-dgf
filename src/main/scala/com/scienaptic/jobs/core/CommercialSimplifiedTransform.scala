...
/*val claimsAndSKUJoinsUnionDF = spark.read.option("header","true").option("inferSchema","true").csv("E:\\Scienaptic\\HP\\Pricing\\Testing\\Commercial_Alteryx\\spark-intermediate\\claims_consolidated.csv")
.withColumn("Week End Date", to_date(col("Week End Date")))
.withColumn("Partner Ship Calendar Date", to_date(col("Partner Ship Calendar Date"))).cache()
val claimsAndMasterCalendarInnerJoinDF = spark.read.option("header","true").option("inferSchema","true").csv("E:\\Scienaptic\\HP\\Pricing\\Testing\\Commercial_Alteryx\\spark-intermediate\\claimsAndMasterCalendarInnerJoinDF.csv")
.withColumn("Week End Date", to_date(col("Week End Date")))
.withColumn("Partner Ship Calendar Date", to_date(col("Partner Ship Calendar Date")))
...
*/
    val claimsProgramFilterPromoOptionCDF = spark.read.option("header","true").option("inferSchema","true").csv("E:\\Scienaptic\\HP\\Pricing\\Testing\\Commercial_Alteryx\\spark-intermediate\\Program_Promo_Option_C.csv")
      .withColumn("Week End Date", to_date(col("Week End Date"))).cache()
    //TODO: check why dropping duplicates required
...
writeDF(claimsWithEndDateChangeDF,"New_End_Date_Unique")
    ...
    /*val claimsWithEndDateChangeDF = spark.read.option("header","true").option("inferSchema","true").csv("E:\\Scienaptic\\HP\\Pricing\\Testing\\Commercial_Alteryx\\spark-intermediate\\New_End_Date_Unique.csv")
      .withColumn("New End Date", to_date(col("New End Date")))
      .withColumn("Max_Week_End_Date", to_date(col("Max_Week_End_Date")))
      .withColumn("Start Date", to_date(col("Start Date")))
      .withColumn("End Date", to_date(col("End Date"))).cache()*/
/*
...
//writeDF(claimsSKUWEDRebateAmountSortDF,"UniquesFromNewEndDate")*/
    ...
    val claimsGroupResellerWEDSKUSumClaimQuantDF = spark.read.option("header","true").option("inferSchema","true").csv("E:\\Scienaptic\\HP\\Pricing\\Testing\\Commercial_Alteryx\\spark-intermediate\\Program_Big_Deal.csv")
      .withColumn("Week End Date", to_date(col("Week End Date"))).cache()
    val claimsSKUWEDRebateAmountSortDF = spark.read.option("header","true").option("inferSchema","true").csv("E:\\Scienaptic\\HP\\Pricing\\Testing\\Commercial_Alteryx\\spark-intermediate\\UniquesFromNewEndDate.csv")
...
//writeDF(masterCalSelectForDistinctDF,"masterCalSelectForDistinctDF")
/* ---------------------------- TIDY HISTORICAL POS ------------------------- */
/*
...
posqtyOutputCommercialDF.write.option("header","true").mode(SaveMode.Overwrite).csv("/etherData/Pricing/outputs/posqty_commercial_output_Feb5.csv")
